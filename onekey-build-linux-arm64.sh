...
build_variant release "$OPENSSL_LIB_RELEASE" linux-aarch64
build_variant debug "$OPENSSL_LIB_DEBUG" linux-aarch64 enable-debug
...
export OPENSSL_LIBS_DEBUG="${OPENSSL_SYMBOL_HIDE_FLAGS} -L\"$OPENSSL_LIB_DEBUG\" -lssl -lcrypto -ldl -lpthread"
export PATH="$SCRIPT_DIR/qtbase/bin:$PATH"

pushd "$SCRIPT_DIR" >/dev/null
./configure -prefix "$INSTALL_DIR" -confirm-license -opensource -debug-and-release -force-debug-info -nomake examples -nomake tests -openssl-linked -platform linux-g++ -I "$OPENSSL_INCLUDE" -L "$OPENSSL_LIB_RELEASE"
make -j"$(cpu_count)"
make install
popd >/dev/null
